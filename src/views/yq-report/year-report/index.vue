...
export default {
  ...
  methods: {
    ...
    /* 获取列表 */
    async getReportList() {
      const { data: res } = await this.$http.get(`/yq/yearReportList/
    ...
    down(report) {
      console.log(report)
      // return
      const res = `${process.env.VUE_APP_API_URL}/yq/exportReportYear/${report.startTimeText}/${report.endTimeText}}`
      window.open(res)
    }
  },
  created() {
