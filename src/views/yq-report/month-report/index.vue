...
export default {
  ...
  methods: {
    ...
    /* 获取列表 */
    async getReportList() {
      const { data: res } = await this.$http.get(`/yq/monthReportList/
    ...
    /* 格式件时间 */
    formt(row) {
      // return window.Bee.DateUtils.formt(row, 'yyyy-MM-dd hh:mm:ss')
    ...
    down(report) {
      console.log(report)
      // return
      const res = `${process.env.VUE_APP_API_URL}/yq/exportReport/${report.startTimeText}/${report.endTimeText}}`
      window.open(res)
    }
  },
  created() {
