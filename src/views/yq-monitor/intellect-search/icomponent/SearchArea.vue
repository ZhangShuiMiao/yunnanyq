      ...
      <el-form :inline="true" name="searchForm" :model="formInline" class="demo-form-inline">
        ...
        <el-form-item label="来源类型" style="display: block">
          <div class="cls-count-warp">
            <div v-for="(item, index) in clsArr" :key="index" class="cls-item" @click="siteClsChoose(item.key, index)">
...
export default {
  ...
  methods: {
    ...mapActions('intellectSearch', ['searchObjChange', 'pageObjChange']),
    siteClsChoose(key, index) {
      this.formInline.siteCls = key
      this.clsArr[index].selected = true
    },
    getClsCount () {
      this.$ynHttp.post('/search/getClsCount', this.formInline).then(res => {
        console.log(res)
        let data = res.data
        if (data.code === '0') {
          this.clsArr = data.data
        }
