  ...
  <div>
    ...
    <list-warp :checkAll="checkAll"  :hideCheckBox="true" @handleCheckAllChange="handleCheckAllChange">
      ...
      <template v-slot:listContent>
        ...
        <list-item v-for="(item,index) in workOrderList" :showCheckBox="false" :type="searchObj.type" :key="index" :itemInfo="item" :index="index" :checked="checkedList.includes(item.id)" @handleCheckOne="handleCheckOne" @showDetail="showDetail">
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    if (from.path === '/orderDetail' && Object.keys(from.query).length !== 0 && [0, 2, 3, 7].includes(from.query.type)) {
  ...
  created() {
    ...
    if (!global.orderListflag) {
      this.initListParam({ type: 0, pageSize: 10, pageNo: 1 })
    }
    this.$commonJs.onScrollTop()
  }
}
